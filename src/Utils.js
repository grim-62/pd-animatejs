...
function getTranslate3d(el) {
  ...
  const valuesParsed = values[1].split(/,\s?/g)

  return valuesParsed.map(c => parseFloat(c.replace('px', '')))
}
...
export const init = (
  ...
  const doAction = (lerp) => {
    if (uniforms.currentScroll.value >= 0) {
      ...
      let sec = uniforms.uSection.value
      if (t.length > sec) {
        if (t.length > sec + 1)
  ...
  const mouseWheel = new ScrollPos()
  const staticScroll = () => {
  ...
  Object.assign(uniforms, {
    ...
    scrollLerp: { value: 0.15 },
    gooey: { value: opts.gooey ? true : false },
    infiniteGooey: { value: false },
    growSize: { value: 4, range: [1, 15] },
    durationOut: { value: 2, range: [.1, 5] },
    durationIn: { value: 2.5, range: [.1, 5] },
    time: { value: 0 },
  ...
  function animate() {
    ...
    uniforms.mousei.value.x = THREE.MathUtils.lerp(uniforms.mousei.value.x, mouseCoords.x, .15)
    uniforms.mousei.value.y = THREE.MathUtils.lerp(uniforms.mousei.value.y, mouseCoords.y, .15)
