...
let globalMouseFollower = null
let picchemousefollower = null
...
export function makeMagnet(element, opts = {}) {
  document.querySelectorAll(element).forEach(function (elem) {
    elem.addEventListener("mousemove", function (dets) {
      ...
      const zeroonex = gsap.utils.mapRange(
        0,
        bcr.width,
        0,
        1,
      ...
      const zerooney = gsap.utils.mapRange(
        0,
        bcr.height,
        0,
        1,
  ...
  switch (opts.style || 1) {
    ...
    case 2:
      alltexts.forEach(function (elem, i) {
        const len = elem.childNodes.length - 1
        for (let i = 0; i < elem.childNodes.length / 2; i++) {
          elem.childNodes[i].dataset.delay = i
        }
        for (
          let i = Math.floor(elem.childNodes.length / 2);
        ...
        elem.childNodes.forEach(function (al) {
          gsap.from(al, {
            y: 20,
            delay: al.dataset.delay * (opts.multiplier || 0.15),
            opacity: 0,
            ease: opts.ease || Expo.easeOut,
          })
        })
    ...
    default:
      console.warn(
        "pdJS : no such style available for text, mentioned in textanimate()"
      )
  }
} //!SECTION
...
export function hoverWithMediaCircle(element, opts) {
  function calculateMedia(indexofelem) {
    const lengthofres = opts.images ? opts.images.length : opts.videos.length
  ...
  document.querySelectorAll(element).forEach(function (elem, index) {
    ...
    let timer
    elem.addEventListener("mousemove", function (dets) {
      const trans = gsap.utils.pipe(
        gsap.utils.clamp(-1, 1),
        gsap.utils.mapRange(-1, 1, 0.8, 1.2)
      )
      const diffx = trans(dets.clientX - prevx)
      const diffy = trans(dets.clientY - prevy)
      prevx = dets.clientX
      prevy = dets.clientY
...
let scene, camera, renderer, container = null
...
export function imageEffect(element = "img", opts = {}) {
  ...
  document.querySelectorAll(element).forEach(function (elem) {
    ...
    switch (opts.style || 1) {
      ...
      case 1: {
        const { debugObj, panel, uniforms, animate } = init(
          elem,
          vertex_1,
          fragment_1, {
      ...
      case 2: {
        const { debugObj, controlKit, panel, uniforms, animate } = init(
          elem,
          vertex_2,
          fragment_2,
          {
      ...
      case 3: {
        const { debugObj, panel, elemMesh, uniforms, animate } = init(
          elem,
          vertex_3,
          fragment_3,
          {
...
// Create Pd object containing all exported functions
const Pd = {
  imageMasker,
  mouseFollower,
  textAnimate,
...
// Exporting Pd as part of the library
export { Pd };